...
module TragicCode
  ...
  class Azure
    ...
    Puppet.info("TragicCode::Azure::get_secrets - Getting secrets from Azure")
      ...
      Puppet.info("TragicCode::Azure::get_secrets - Initial secrets found: #{secrets_res}")
      next_page = JSON.parse(res.body)['nextLink']
      while not next_page.empty?
        Puppet.info("TragicCode::Azure::get_secrets - Getting next page: #{next_page}")
        uri = URI(next_page)
        req['Authorization'] = "Bearer #{access_token}"
        res = Net::HTTP.start(uri.hostname, uri.port, use_ssl: true) do |http|
          http.request(req)
        ...
        raise res.body unless res.is_a?(Net::HTTPSuccess)
        Puppet.info("TragicCode::Azure::get_secrets - Adding secrets: #{JSON.parse(res.body)['value']}")
        secrets_res = secrets_res + JSON.parse(res.body)['value']
        next_page = JSON.parse(res.body)['nextLink']
      end
      Puppet.info("TragicCode::Azure::get_secrets - Found secrets: #{secrets_res}")
      return secrets_res
    end
  end
end
